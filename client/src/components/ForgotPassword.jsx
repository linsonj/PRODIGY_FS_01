...
export default function ForgotPassword({setForgotPass}) {
    ...
    const {setResetPass,resetPass}=useContext(ProtectedContext);
    function reqForgotPassword(){
        const emailval=emailRef.current.value;
        if(emailval!==''){
            console.log('email value is');
            console.log(emailval);//req to api
            fetchApi('/forgot-password',{email:emailval},()=>{
                console.log('success');
                alert('A password reset link is send to your email, its valid for 5 minutes')
                setForgotPass(false);
                setResetPass(true)
